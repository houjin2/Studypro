...
void  USHUD::BindStatComponent(class USStatComponent* InStatComponent)
{
	if (true == ::IsValid(InStatComponent))
	{
		...
		//PlayerController::Beginplay()가 SStatComponent::BeginPlay()  보다 먼저 호출되기 때문에 아래와 같이 초기화.
		USGameInstance* GameInstance = Cast<USGameInstance>(GetWorld()->GetGameInstance());
		if (true == ::IsValid(GameInstance))
		{
			if (nullptr != GameInstance->GetCharacterStatDataTable() || GameInstance->GetCharacterStatDataTableRow(1))
			{
...
void USHUD::BindPlayerState(class ASPlayerState* InPlayerState)
{
